...
export default function Settings() {
  ...
  const handleTrash = async (id) => {
    // Custom toast-based confirmation is cleaner than window.confirm
    toast(
      (t) => (
        <span className="flex items-center space-x-4">
          <b>Archive this rule?</b>
          <button
  ...
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center mt-20 space-y-4 text-slate-400">
      ...
      <div className="flex justify-between items-center mb-10">
        ...
        <div className="flex space-x-4">
          <button
            ...
            className="flex items-center space-x-2 px-5 py-2.5 rounded-2xl bg-white border border-slate-200 text-slate-600 font-bold hover:bg-slate-50 transition-all shadow-sm"
          >
            {showTrash ? (
              <>
                <ArrowLeft size={16} /> <span>Active Rules</span>
              </>
            ) : (
              <>
                <Trash2 size={16} /> <span>View Trash</span>
              </>
            )}
          </button>
          ...
          {!showTrash && (
            <button
              ...
              className="flex items-center space-x-2 px-6 py-2.5 rounded-2xl bg-blue-600 text-white font-bold hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 transition-all"
            >
        ...
        <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">
              <tr>
                <th className="px-8 py-5">Core Principle</th>
                <th className="px-8 py-5 text-center">Rigidity</th>
                  ...
                  <td className="px-8 py-6 text-center">
                    <span
                      className={`px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase border ${
                        (rule.rule_type || rule.type)?.toLowerCase() === "hard"
          ...
          {rules.length === 0 && (
            <div className="p-20 text-center space-y-4">
              <Shield className="mx-auto w-12 h-12 text-slate-200" />
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleCreate} className="p-8 space-y-6">
              <div className="space-y-2">
                ...
                <textarea
                  required
                  className="w-full p-5 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm min-h-[120px] resize-none"
              ...
              <div className="flex space-x-4 pt-4">
                <button
                  ...
                  className="flex-1 px-4 py-4 bg-white border border-slate-200 text-slate-500 rounded-2xl hover:bg-slate-50 transition font-black text-xs uppercase tracking-widest"
                >
                  Discard
                </button>
                <button
                  type="submit"
                  className="flex-1 px-4 py-4 bg-slate-900 text-white rounded-2xl hover:bg-blue-600 shadow-xl shadow-blue-200/20 transition-all font-black text-xs uppercase tracking-widest"
                >
                  Establish
                </button>
              </div>
